    ...
    public partial class TakeSpareAdd : Form
    {
        ...
        private void GetSpareAdd_Load(object sender, EventArgs e)
        {
            ...
            if (this.id != "")
            {
                ...
                string cusid = "";
                string verid = "";
                string selectOne1 = "SELECT verify.*,get_id,get_date,customers.veh_id,customers.veh_type from verify " +
                    "INNER JOIN get_spares on get_spares.ver_id = verify.ver_id  join customers on customers.cus_id = verify.cus_id  WHERE get_spares.get_id = @id LIMIT 1";
            ...
            {
                string id2 = "";
                string query2 = "Select case when Max(substr(get_id, -6)) + 1 is null then 'GET-000001' else case when (Max(substr(get_id, -6)) + 1) < 10 then CONCAT('GET-00000',(Max(substr(get_id, -6)) + 1)) else CONCAT('GET-0000',(Max(substr(get_id, -6)) + 1)) end end as MaxID from get_spares";
